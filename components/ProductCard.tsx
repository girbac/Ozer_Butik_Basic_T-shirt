  ...
  return (
    <article className="group @container flex flex-col">
      ...
      <div className="mt-3 flex flex-col">
        ...
        {product.tagline && (
          <p className="mt-1 hidden text-xs text-ink-muted @min-[240px]:block">
            {product.tagline}
          </p>
        )}

        {product.colors.length > 1 && (
          <div className="mt-3 flex items-center gap-2" role="group" aria-label="Renk seçimi">
              ...
              {color.sizes.map((size) => {
                ...
                return (
                  <button
                    ...
                    className={`h-10 min-w-10 border px-2 text-xs font-medium transition-colors ${
                      outOfStock
                        ? "cursor-not-allowed border-line text-disabled line-through"
                        : "border-line hover:border-ink hover:bg-ink hover:text-bg"
                    }`}
                  >
                    {size.size}
                  </button>
